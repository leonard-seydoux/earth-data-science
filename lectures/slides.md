...
Here, we have a dataset with samples $\mathbf{x} \in \mathbb{R}^2$ and labels $\mathbf{y} \in \{0, 1\}$.

The model should allow to predict the category of a new sample $\mathbf{x}$. We can then observe the boundary decision.

Depending on the model, the loss, and the regularization, the boundary decision can be linear or non-linear.
